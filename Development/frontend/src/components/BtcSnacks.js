...
import BtcSnackAbi from "../abis/BtcSnacks.json";
import { BtcSnackAddress } from "./addresses.js";
...
class BtcSnacks extends Component {
  ...
  componentDidMount() {
    try {
      ...
      if (window.ethereum) {
        ...
        const contract = new ethers.Contract(BtcSnackAddress, BtcSnackAbi.abi, provider);

        // Get signer & contract for next calls
        const contractWithSigner = new ethers.Contract(BtcSnackAddress, BtcSnackAbi.abi, provider.getSigner());
