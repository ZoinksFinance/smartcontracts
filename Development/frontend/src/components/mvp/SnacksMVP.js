...
import { SnacksAddress, ZoinksTokenAddress } from "./../addresses.js";
import TransactionInfo from "./../common/TransactionInfo.js";
import BlockchainUtils from "./../common/BlockchainUtils.js";

class SnacksMVP extends Component {

  constructor(props) {
    super(props);
    this.utils = null;
    this.utilsZoinks = null;
    this.state = {
  ...
  componentDidMount() {
    try {
      ...
      if (window.ethereum) {
        ...
        const contractZoinksWithSigner = new ethers.Contract(ZoinksTokenAddress, ZoinksTokenAbi.abi, provider.getSigner());
        ...
        this.utilsZoinks = new BlockchainUtils(contractZoinksWithSigner, null, this.transactionProcessing, false, this, provider);
      } else {
      ...
      const msgError2 = `Error in Snacks componentDidMount: ${error}`;
    ...
    return (
      <>
        ...
        <form className="form" action="#" onSubmit={async (event) => {
          ...
          const sender = SnacksAddress;
          await this.utilsZoinks.methodSend("approve", sender, amountSnacksToMint);
