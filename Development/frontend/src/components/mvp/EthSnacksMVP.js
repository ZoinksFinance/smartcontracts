...
import EthSnacksAbi from "../../abis/EthSnacks.json";
import BEP20Abi from "../../abis/BEP20.json";
import { EthSnackAddress, ETHAddress } from "./../addresses.js";
import TransactionInfo from "./../common/TransactionInfo.js";
import BlockchainUtils from "./../common/BlockchainUtils.js";

class EthSnacksMVP extends Component {
  ...
  componentDidMount() {
    try {
      ...
      if (window.ethereum) {
        ...
        const contract = new ethers.Contract(EthSnackAddress, EthSnacksAbi.abi, provider);

        // Get signer & contract for next calls
        const contractWithSigner = new ethers.Contract(EthSnackAddress, EthSnacksAbi.abi, provider.getSigner());
        const contractETHWithSigner = new ethers.Contract(ETHAddress, BEP20Abi.abi, provider.getSigner());
        ...
        this.utilsETH = new BlockchainUtils(contractETHWithSigner, null, this.transactionProcessing, false, this, provider);
      } else {
      ...
      const msgError2 = `Error in EthSnacks componentDidMount: ${error}`;
    ...
    return (
      <>
        ...
        <form className="form" action="#" onSubmit={async (event) => {
          ...
          const sender = EthSnackAddress;
          await this.utilsETH.methodSend("approve", sender, amountSnacksToMint);
