...
import { SnacksAddress } from "./addresses.js";
...
class Snacks extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      totalSupply: null,
      undistributedFeeBTCSnacks: null,
      undistributedFeeETHSnacks: null,
      undistributedFeeSnacks: null,
      zoinksStakingPool: null,
  ...
  componentDidMount() {
    try {
      ...
      if (window.ethereum) {
        ...
        const contract = new ethers.Contract(SnacksAddress, SnacksAbi.abi, provider);

        // Get signer & contract for next calls
        const contractWithSigner = new ethers.Contract(SnacksAddress, SnacksAbi.abi, provider.getSigner());
  ...
  async readProps(method) {
    try {
      ...
      const allProps = {
        ...
        undistributedFeeBTCSnacks: await this.state.contract.undistributedFeeBTCSnacks(),
        undistributedFeeETHSnacks: await this.state.contract.undistributedFeeETHSnacks(),
        undistributedFeeSnacks: await this.state.contract.undistributedFeeSnacks(),
        zoinksStakingPool: await this.state.contract.zoinksStakingPool(),
      };
      ...
      this.setState({
        ...
        undistributedFeeBTCSnacks: allProps.undistributedFeeBTCSnacks.toString(),
        undistributedFeeETHSnacks: allProps.undistributedFeeETHSnacks.toString(),
        undistributedFeeSnacks: allProps.undistributedFeeSnacks.toString(),
        zoinksStakingPool: allProps.zoinksStakingPool.toString(),
      });
    } catch (error) {
      ...
      <>
        <div className="container">
          <div className="propsContainer">
            ...
            <p>undistributedFeeBTCSnacks</p>
            <p><span>{this.state.undistributedFeeBTCSnacks ?? "Empty"}</span></p>
            <p>undistributedFeeETHSnacks</p>
            <p><span>{this.state.undistributedFeeETHSnacks ?? "Empty"}</span></p>
            <p>undistributedFeeSnacks</p>
            <p><span>{this.state.undistributedFeeSnacks ?? "Empty"}</span></p>
            <p>zoinksStakingPool</p>
            <p><span>{this.state.zoinksStakingPool ?? "Empty"}</span></p>
          </div>
